...
void Personnage::moving_cible(TileMap& map){
    sf::Vector2f cibler_pos(cible->sprite.getPosition().x + (PERSO_WIDTH/2), cible->sprite.getPosition().y + (PERSO_HEIGHT/2));
    sf::Vector2f pos(sprite.getPosition().x + (PERSO_WIDTH/2), sprite.getPosition().y + (PERSO_HEIGHT/2));

    int dir = getDir(pos, cibler_pos);
    if( ((dir == 0 || dir == 3) && cibleDisctance(*cible) <= PERSO_WIDTH + speed) || ((dir == 1 || dir == 2) && cibleDisctance(*cible) <= PERSO_HEIGHT + speed)){
        m_moving = false;
    }else{
        m_moving = true;
    }
